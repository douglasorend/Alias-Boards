...
// Fields to add
$new_fields = array(
	'alias_cat' => array('name'=> 'alias_cat', 'type'=>'VARCHAR(255)'),
	'alias_child' => array('name'=> 'alias_child', 'type'=>'VARCHAR(255)'),
);
...
foreach ($new_fields as $column_name => $column_attributes)
{
	...
	if(!in_array($column_name, $table_columns))
		$smcFunc['db_add_column']($db_prefix . "boards", $column_attributes);
}
